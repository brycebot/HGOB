...
int gOutputPin = 7;    // digital pin 0 - check the pin diagram in the IDE
float gInterval = 2; // how often to toggle the LED (in seconds)
 ...
 */
    ...
    if (strncmp(source, "bela_digitalOut", prefixLength) == 0)
    {
        ...
        { //the two ifs are used instead of if(strlen(source) >= prefixLength+2)
            if (source[prefixLength + 1] != 0)
            {
                ...
                if (channel < gDigitalChannelsInUse)
    ...
    for (unsigned int tick = 0; tick < numberOfPdBlocksToProcess; ++tick)
    {
        ...
        if (gDigitalEnabled)
        {
            ...
            for (j = 0, p0 = gInBuf; j < gLibpdBlockSize; j++, p0++)
            {
                ...
                {
                    if (dcm.isSignalRate(k) && dcm.isInput(k))
                    { // only process input channels that are handled at signal rate
        ...
        if (gDigitalEnabled)
        {
            ...
            for (j = 0, p0 = gOutBuf; j < gLibpdBlockSize; ++j, ++p0)
            {
                ...
                {
                    if (dcm.isSignalRate(k) && dcm.isOutput(k))
                    { // only process output channels that are handled at signal rate
        ...
        */
        ...
        {
            if (gCount == (int)(context->digitalSampleRate * gInterval))
